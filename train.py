    ...
    while True:  # cur_itrs < opts.total_itrs:
        ...
        for (images, labels) in train_loader:
            ...
            if (cur_itrs) % opts.val_interval == 0:
                if opts.bilinear:
                    save_ckpt('checkpoints/latest_%s.pth' % 'biUS')
                else:
                    save_ckpt('checkpoints/latest_%s.pth' % 'TCUS')
                print("validation...")
                model.eval()
                val_score, ret_samples = validate(
                ...
                if val_score['Mean IoU'] > best_score:  # save best model
                    ...
                    if opts.bilinear:
                        save_ckpt('checkpoints/best_BiUS.pth')
                    else:
                        save_ckpt('checkpoints/best_TCUS.pth')
